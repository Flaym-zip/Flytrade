...
class Academy08(Academy06):
    def __init__(self, data, expected_source=None):
        ...
        self.db.execute('CREATE TABLE IF NOT EXISTS exposed_test(uid TEXT PRIMARY KEY, session TEXT)')
        # Backfill existing sessions: opening an old test is still an exposure.
        with self.db:
            if self.test_opened and self.splits:
                self._mark_seen(self.splits.get('test', []), self.session)
            for (raw,) in self.db.execute('SELECT payload FROM archives'):
                d = json.loads(raw)
                if d.get('test_opened') and d.get('splits'):
                    self._mark_seen(d['splits'].get('test', []), d.get('session'))
        self.refresh_usage()

    def checkpoint(self):
        d = super().checkpoint()
        d.update(version='0.8.0-alpha', lineage=copy.deepcopy(self.lineage))
        return d
    ...
    def import_lines(self, lines, origin='upload'):
        result=super().import_lines(lines, origin)
        self.refresh_usage()
        return result

    def _mark_seen(self, ids, session):
        self.db.executemany('INSERT OR IGNORE INTO exposed_test VALUES (?,?)',
                            [(uid, str(session)) for uid in ids])
    ...
    def _build(self, config):
        ...
        for (raw,) in self.db.execute('SELECT payload FROM corpus ORDER BY placed DESC LIMIT ?', (config.max_windows,)):
        ...
        return plan, {k:[e.uid for e in v] for k,v in splits.items()}, report, considered, len(eps)

    def preview(self, config):
        plan,splits,report,n,eligible=self._build(config)
    ...
    def _start_session(self, config, plan, splits, report, mode, retain=False):
    ...
    def create(self, config):
        ...
        plan,splits,report,_,_=self._build(config)
    ...
    def replay(self):
        ...
        return self._start_session(self.config,plan,self.splits,self.split_report,'replay')
    ...
    def continue_training(self, passes=1):
        ...
        return self._start_session(config,plan,self.splits,self.split_report,'continue',retain=True)
    ...
    def open_test(self):
        ...
        try:
            ...
            if self.test_opened:raise ValueError('Test deja ouvert dans cette experience.')
            self.plan.extend([TEST,uid] for uid in self.splits['test']);self.test_opened=True
            with self.db:
                self._mark_seen(self.splits['test'],self.session)
                self.db.execute('INSERT OR REPLACE INTO state VALUES (1,?)',(canonical(self.checkpoint()),))
        except Exception:
